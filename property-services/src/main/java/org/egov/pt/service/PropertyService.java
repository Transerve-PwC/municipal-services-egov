	...
	public void importProperties(InputStream file, InputStream matchedFile, Long skip, Long limit) throws Exception {
		...
		excelService.read(file, skip, limit, (RowExcel row) -> {
			...
			try {
				...
				RequestInfo userCreateRequestInfo = RequestInfo.builder().action("_create").apiId("Rainmaker").did("1")
						.key("").msgId("20170310130900|en_IN").ver(".01").build();
				...
				String name = legacyRow.getOwnerName() != null && legacyRow.getOwnerName() != ""
						? legacyRow.getOwnerName()
				...
				Map<String, String> tokenResponse = (Map<String, String>) userService.getToken(user.getUserName(),
						"123456", tenantId, user.getType());
				String token = (String) tokenResponse.get("access_token");

				RequestInfo requestinfo = RequestInfo
						.builder().authToken(token).action("token").apiId("Rainmaker").did("1").key("")
						.msgId("20170310130900|en_IN").ver(".01").userInfo(org.egov.common.contract.request.User
								.builder().type(user.getType()).tenantId("up").userName(user.getUserName()).build())
						.build();
				...
				String ackNo = propertyutil
						...
						.get(0);
				// String ackNo = propertyutil.getIdList(requestInfo, tenantId,
				...
				System.out
						.println("Row[" + row.getRowIndex() + "] - [" + legacyRow.toString() + "] -" + e.getMessage());
			}
			return true;
		});
